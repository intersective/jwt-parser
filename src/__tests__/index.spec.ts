...
beforeEach(() => {
  token = 'ewoJInR5cGUiOiAiSldUIiwKCSJhbGciOiAiUlMyNTYiCn0.' +
...
it('1. it should return the correct values [TypeScript]', () => {
  // sign with RSA SHA256
  res = jwtParse(token, envVariables, 'TEST');
});

it('1.1 it should return the correct values [JavaScript]', () => {
  // sign with RSA SHA256
  const jwtParse = require('../../lib');
  res = jwtParse(token, envVariables, 'TEST');
});
